...
var (
	...
	courseTeacherRowsExpectAutoSet   = strings.Join(stringx.Remove(courseTeacherFieldNames, "`create_at`", "`create_time`", "`created_at`", "`update_at`", "`update_time`", "`updated_at`"), ",")
...
func (m *defaultCourseTeacherModel) Insert(ctx context.Context, data *CourseTeacher) (sql.Result, error) {
	...
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.Id, data.CourseId, data.TeacherId, data.IsShow, data.CIndex, data.DepId, data.Creater, data.Updater, data.Deleted)
	}, courseTeacherIdKey)
	return ret, err
}
