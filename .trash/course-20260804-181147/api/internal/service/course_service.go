...
func (s *courseService) SaveBaseInfo(ctx context.Context, req *types.CourseBaseInfoSaveDTO, userId int64) (*types.CourseSaveVO, error) {
	...
	contentDraft := &model.CourseContentDraft{
		CourseIntroduce: sql.NullString{String: req.Introduce, Valid: true},
		CourseDetail:    sql.NullString{String: req.Detail, Valid: true},
		UsePeople:       sql.NullString{String: req.UsePeople, Valid: true},
	}
	...
	if req.Id == 0 {
		...
		if _, err := s.courseDraftModel.Insert(ctx, draft); err != nil {
			return nil, xerr.Wrap(err, xerr.CodeInternal, "新增课程草稿失败")
		}
		if _, err := s.courseContentDraftModel.Insert(ctx, contentDraft); err != nil {
	...
	if see {
		...
		if course != nil {
			...
			vo = &types.CourseBaseInfoVO{
				...
				CoureScore:    float64(course.Score) / 10,
				...
				Score:         int32(course.Score),
				PurchaseStartTime: formatNullTime(course.PurchaseStartTime),
				PurchaseEndTime:   course.PurchaseEndTime.Format(timeFormat),
			}
			if content != nil {
				vo.Detail = content.CourseDetail.String
				vo.Introduce = content.CourseIntroduce.String
				vo.UsePeople = content.UsePeople.String
			}
		}
	}
	if vo == nil {
