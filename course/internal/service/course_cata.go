...
func (s *courseService) packageCatas(ctx context.Context, courseId int64, list []*types.CataSaveDTO, shelfCatas []*model.CourseCatalogue) ([]*model.CourseCatalogueDraft, error) {
	...
	for _, sc := range shelfCatas {
		savedMap[sc.Id] = &model.CourseCatalogueDraft{
			...
			Type:              sc.Type,
			ParentCatalogueId: sc.ParentCatalogueId,
			MediaId:           sc.MediaId,
			VideoId:           sc.VideoId,
			VideoName:         sc.VideoName,
			LivingStartTime:   sc.LivingStartTime,
			LivingEndTime:     sc.LivingEndTime,
	...
	for _, cata := range list {
		...
		chapter.ParentCatalogueId = 0
		chapter.CIndex = int64(cata.Index)
		...
		for _, section := range cata.Sections {
			...
			item.ParentCatalogueId = chapterId
			if section.Type == int32(CataTypeSection) {
				sectionIndex++
				item.CIndex = sectionIndex
			}
	...
	if !isNoUpShelf {
		for _, c := range catas {
			if c.Type == CataTypeSection && c.CanUpdate == 0 {
				if c.CIndex > chapterIdAndMaxSection[c.ParentCatalogueId] {
					chapterIdAndMaxSection[c.ParentCatalogueId] = c.CIndex
				}
			}
		}
		for _, c := range catas {
			if c.Type == CataTypeChapter && c.CanUpdate == 0 && c.CIndex > maxChapterIndex {
				maxChapterIndex = c.CIndex
			}
		}
	}
...
func computeMaxIndexOnShelf(c *model.CourseCatalogueDraft, maxSection map[int64]int64, maxChapter int64) int64 {
	if c.Type == CataTypeSection {
		return maxSection[c.ParentCatalogueId]
	}
...
func (s *courseService) SaveMedia(ctx context.Context, courseId int64, list []*types.CourseMediaSaveDTO) error {
	...
	for _, m := range list {
		items = append(items, &model.CourseCatalogueDraft{
			...
			VideoId:       sql.NullInt64{Int64: m.MediaId, Valid: m.MediaId > 0},
	...
	// 4.统计并更新每个章的媒资总时长
	durations, err := s.cataDraftModel.SumMediaDurationByCourse(ctx, courseId)
	if err != nil {
		return xerr.Wrap(err, xerr.CodeInternal, "统计章节时长失败")
	}
	updates := make([]*model.CourseCatalogueDraft, 0, len(durations))
	for parentId, sum := range durations {
		updates = append(updates, &model.CourseCatalogueDraft{Id: parentId, MediaDuration: sum})
	}
	if err := s.cataDraftModel.UpdateMediaDuration(ctx, updates); err != nil {
...
func (s *courseService) SaveSubjects(ctx context.Context, courseId int64, list []*types.CataSubjectDTO) error {
	...
	for _, c := range list {
		for _, subjectId := range c.SubjectIds {
			if err := s.cataSubjectDraftModel.Insert(ctx, &model.CourseCataSubjectDraft{
				Id:        idgen.NextID(),
				CourseId:  courseId,
				CataId:    c.CataId,
				SubjectId: subjectId,
...
func (s *courseService) GetSubjects(ctx context.Context, courseId int64) ([]*types.CataSimpleSubjectVO, error) {
	...
	for _, sub := range subjects {
		nameMap[sub.Id] = sub.SubjectName
	}
	// 按目录分组
